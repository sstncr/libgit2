	...
	*out = NULL;
	...
	if (stack) {
		...
		if ((error = git_str_joinpath(&path, parent_directory, "reftable")) < 0 ||
			...
			refdb_reftable_stack_free(stack);
			error = refdb_reftable_error(error, "failed loading stack");
			goto out;
		}
	}
...
static int refdb_reftable_lookup(git_reference **out_ref,
				 git_refdb_backend *_backend,
				 const char *refname)
{
	refdb_reftable *backend = GIT_CONTAINER_OF(_backend, refdb_reftable, parent);
	...
	if ((error = refdb_reftable_reference_from_record(out_ref, &ref, backend->repo->oid_type)) < 0)
		goto out;
				    ...
				    const char *glob, bool is_worktree)
{
	...
	while (1) {
		...
		switch (which) {
		case REFDB_REFTABLE_STACK_MAIN:
			if (is_worktree && git_reference__is_per_worktree_ref(it->ref.refname))
				continue;
			break;
		case REFDB_REFTABLE_STACK_WORKTREE:
			if (!git_reference__is_per_worktree_ref(it->ref.refname))
				continue;
			break;
		}
							      ...
							      size_t *out_nr)
{
	...
	while (1) {
		...
		if (new_name) {
			...
			/* Pass ownership of the record to `updates`. */
				      ...
				      const char *name)
{
	...
	while (1) {
		...
		if ((error = git_vector_insert(&reflog->entries, entry)) < 0) {
			git_reflog_entry__free(entry);
			goto out;
		}
	}
