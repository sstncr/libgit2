	...
	uint32_t flags)
{
	...
	*out = false;
	...
	if (!(flags & GIT_PATH_REJECT_NOTHING)) {
	...
	uint32_t flags)
{
	...
	for (;;) {
		...
		if ((error = git_fs_path_dirname_r(&path, path.ptr)) < 0)
			goto out;
		if (!(flags & GIT_REPOSITORY_OPEN_NO_DOTGIT) &&
		    (error = git_fs_path_dirname_r(&path, path.ptr)) < 0)
			goto out;
		...
		const char *ceiling_dirs)
{
	...
	error = find_repo(&paths, start_path, ceiling_dirs, flags);
	if (gitdir_path && error == 0)
		error = git_buf_fromstr(gitdir_path, &paths.gitdir);
	if (workdir_path && error == 0)
		error = git_buf_fromstr(workdir_path, &paths.workdir);
	if (gitlink_path && error == 0)
		error = git_buf_fromstr(gitlink_path, &paths.gitlink);
	if (commondir_path && error == 0)
		error = git_buf_fromstr(commondir_path, &paths.commondir);
