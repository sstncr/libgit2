	...
	const char *refname)
{
	...
	if ((error = git_branch__remote(out, &remote, repo, refname)) < 0)
		return error;
...
int git_branch_remote(
	...
	const char *refname)
{
	GIT_BUF_WRAP_PRIVATE(name_out, git_branch__remote, remote_out, repo, refname);
}

static int git_branch__push_remote(
	git_str* name_out,
	git_remote** remote_out,
	git_repository *repo,
	const char *refname)
...
int git_branch_push_remote(
	...
	const char *refname)
{
	GIT_BUF_WRAP_PRIVATE(name_out, git_branch__push_remote, remote_out, repo, refname);
}
